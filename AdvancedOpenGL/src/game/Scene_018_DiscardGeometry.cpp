...
void Scene_018_DiscardGeometry::draw()
{
    /*  NOT WORKING
        ...
        Matrix4 proj_matrix = Matrix4::createPerspectiveFOV(45.0f, game->windowWidth, game->windowHeight, 0.1f, 1000.0f);
        Matrix4 mv_matrix = Matrix4::createTranslation(Vector3(0.0f, 0.0f, -20.0f)) *
                                Matrix4::createRotationY(f * 5.0f) *
                                Matrix4::createRotationX(f * 100.0f);

        shader.setMatrix4("mvpMatrix", proj_matrix * mv_matrix);
        shader.setMatrix4("mvMatrix", mv_matrix);
