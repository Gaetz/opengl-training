...
void Game::load()
{
	...
	for (int i = 0; i < 10; i++)
	{
		for (int j = 0; j < 10; j++)
		{
			...
			pos.z += 100.0f;
			l->setPosition(pos);
			PointLightComponent* pl = new PointLightComponent(l);
			Vector3 color;
			switch ((i + j) % 5)
			{
			case 0:
			...
			pl->diffuseColor = color;
			pl->innerRadius = 100.0f;
			pl->outerRadius = 200.0f;
		}
	}
	...
	// Setup lights
	LevelLoader::loadLevel("Res\\Levels\\Level0.gplevel");
