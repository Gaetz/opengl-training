...
void MoveComponent::update(float dt)
{
	...
	if (!Maths::nearZero(forwardSpeed))
	{
		...
		if (newPosition.x < -WINDOW_WIDTH / 2) { newPosition.x = WINDOW_WIDTH / 2; }
		else if (newPosition.x > WINDOW_WIDTH / 2) { newPosition.x = -WINDOW_WIDTH / 2; }
		if (newPosition.y < -WINDOW_HEIGHT / 2) { newPosition.y = WINDOW_HEIGHT / 2; }
		else if (newPosition.y > WINDOW_HEIGHT / 2) { newPosition.y = -WINDOW_HEIGHT / 2; }
